...
class ProgramsDatabase:
  ...
  def _register_program_in_island(
    ...
    if score > self._best_score_per_island[island_id]:
      ...
      # Save the best program to a file name in a folder name with current time.
      #create a folder with the current time
      #remove all file under score_code
      p = pathlib.Path(f"./score_code")
      if p.exists():
        for f in p.iterdir():
          if f.is_file():
            f.unlink()
      p = pathlib.Path(f"./score_code/{str(int(time.time()))}")
      if not p.exists():
        p.mkdir(parents=True, exist_ok=True)
      with open( f"./score_code/{str(int(time.time()))}/code_{score}.log", "a") as f:
